...
def main(image_direc='image',
    ...
    for i, img_filename in enumerate(imgs):
        ...
        tic = time.clock()
        # actual work
        sw.calc_seg(nEMIters=num_EM_iters, nItersInner=num_inner_iters, calc_s_cov=calc_s_cov,
        ...
        fname_res_border = '_'.join(
            [img_num, 'n', '{0:03}'.format(nPixels_on_side), 'std', '{0:02}'.format(i_std), 'border', grid + '.png'])
        fname_res_cartoon = '_'.join(
            [img_num, 'n', '{0:03}'.format(nPixels_on_side), 'std', '{0:02}'.format(i_std), 'mean', grid + '.png'])
        ...
        mat_filename = os.path.join(save_path_root  # , img_num + '_std_'+str(i_std)+'.mat')
                                    , '_'.join(
                [img_num, 'n', '{0:03}'.format(nPixels_on_side), 'std', '{0:02}'.format(i_std) + '.mat']))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--nPixels_on_side", type=int,
                        help="the desired number of pixels on the side of a superpixel",
                        nargs='?',
                        const=10, default=10)
    parser.add_argument("--i_std", type=int,
                        help="std dev for color Gaussians, in [5,40]. A smaller value leads to more irregular superpixels",
                        nargs='?',
                        const=15, default=15)
